var CmsUtility = (function() {
	...
	function getAjaxCall(subApiUrl, postData, succCallback, errorCallback, type, isAsync, dataType) {
		var type = type || "get";
		var dataType = dataType || "json";
		var isAsync = isAsync || true;
		var fullUrl = CmsConfig.ServiceUrl.ApiRootUrl + subApiUrl;
			...
			success: function(data) {
				for (var i = 0; i < data.data.length; i++) {
					if (p == data.data[i].provinceCode) {
						str += '<option value="' + data.data[i].provinceCode + '" selected="">' + data.data[i].provinceName +
							'</option>'
					} else {
		...
		for (var i = 0; i < hangyeData3.length; i++) {
			if (data == hangyeData3[i].value) {
				for (var j = 0; j < hangyeData3[i].children.length; j++) {
					str += '<option value="' + hangyeData3[i].children[j].value + '">' + hangyeData3[i].children[j].text +
						'</option>'
				}
			}
		}
				...
				for (var j = 0; j < hangyeData3[i].children.length; j++) {
					if (data1 == hangyeData3[i].children[j].value) {
						for (var m = 0; m < hangyeData3[i].children[j].children.length; m++) {
							str += '<option value="' + hangyeData3[i].children[j].children[m].value + '">' + hangyeData3[i].children[j].children[
								m].text + '</option>'
			...
			function(data) {
				if (data.code == 1000) {
					...
					for (var i = 0; i < data.data.PaymentChannellist.length; i++) {
						str += '<option value="' + data.data.PaymentChannellist[i].id + '">' + data.data.PaymentChannellist[i].paymentchannelname +
							'</option>'
					}
					$('#paymentNo').html(str)
				}
